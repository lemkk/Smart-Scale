...
#define GAP_RECORD_KEY   0x0002 // FDS记录key
...
// 全局变量
// ============================================================
static float g_gap_value = 431.0f;       /**< HX711 重量换算系数（每克对应的原始值差）*///419
...
static float g_temperature = 22.0f;  /**< 当前温度（℃），默认22℃，后续由传感器更新 */
...
static void system_off(void)
{
    ...
    nrf_delay_ms(500);
    app_timer_stop(m_bat_low_blink_timer); // 停止计时器
    nrf_gpio_pin_set(LED_R);               // 熄灭红灯
    ...
    if (m_conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        ...
        while (m_conn_handle != BLE_CONN_HANDLE_INVALID && timeout > 0)
        {
            idle_state_handle();
            timeout--;
        }
...
//存储gapvalue到Flash
static void fds_gapvalue_save(float gap)
{
    static uint32_t data;
    memcpy(&data, &gap, sizeof(float));  // float按位转uint32_t
    ...
    record.file_id           = TARE_FILE_ID;
    record.key               = GAP_RECORD_KEY;
    record.data.p_data       = &data;
    record.data.length_words = 1;

    g_fds_write_done = false;

    if (fds_record_find(TARE_FILE_ID, GAP_RECORD_KEY, &desc, &tok) == NRF_SUCCESS)
        fds_record_update(&desc, &record);
    else
        fds_record_write(&desc, &record);

    NRF_LOG_INFO("GapValue saved to flash: " NRF_LOG_FLOAT_MARKER, NRF_LOG_FLOAT(gap));
}

//读取flash中gapvalue的值
static bool fds_gapvalue_load(float * p_gap)
{
    fds_record_desc_t   desc = {0};
    fds_find_token_t    tok  = {0};
    fds_flash_record_t  flash_record;
    ...
    if (fds_record_find(TARE_FILE_ID, GAP_RECORD_KEY, &desc, &tok) == NRF_SUCCESS)
    {
        if (fds_record_open(&desc, &flash_record) == NRF_SUCCESS)
        {
            uint32_t raw = *(uint32_t *)flash_record.p_data;
            memcpy(p_gap, &raw, sizeof(float));  // uint32_t还原为float
            fds_record_close(&desc);
            NRF_LOG_INFO("GapValue loaded from flash: " NRF_LOG_FLOAT_MARKER, NRF_LOG_FLOAT(*p_gap));
            return true;
        }
    }
    return false;
...
static void meas_step_handler(void * p_context)
{
    ...
    switch (g_meas_phase)
    {
        ...
        case MEAS_PHASE_ADC_SAMPLE:
        {
            ...
            app_timer_start(m_meas_step_timer, APP_TIMER_TICKS(150), NULL);
            break;
        }
        ...
        case MEAS_PHASE_HX_3:
        {
            ...
            NRF_LOG_INFO("Raw data: %ld", (long)(g_hx_sum / 3.0f) + Weight_Maopi); //标定时用
            float weight_f = g_hx_sum / 3.0f / g_gap_value;
            float dt = g_temperature - 22.0f;
            float temp_factor = 1.0f + 0.000510f * dt - 0.0000230f * dt * dt;
            if (temp_factor > 0.0f) weight_f /= temp_factor;
            long weight_g = (long)weight_f;
            long gram = labs(weight_g);
            long deci = (long)(fabsf(weight_f) * 10.0f) % 10;
            if (gram < 0) gram = -gram;
            ...
            if (weight_g < 0)
            {
                len = snprintf(buf, sizeof(buf), "-%ld.%ld g BAT:%d%%\r\n", gram, deci, g_bat_percent);
                NRF_LOG_INFO("Weight: -%ld.%ld g", gram, deci);
            }
            else
            {
                len = snprintf(buf, sizeof(buf), "%ld.%ld g BAT:%d%%\r\n", gram, deci, g_bat_percent);
...
static void nus_data_handler(ble_nus_evt_t * p_evt)
{
    if (p_evt->type == BLE_NUS_EVT_RX_DATA)
    {
        ...
        memcpy(buf, p_evt->params.rx_data.p_data, len);
        buf[len] = '\0';

        if (strncmp((char *)buf, "CAL:", 4) == 0)
        {
            float new_gap = (float)atof((char *)buf + 4);
            if (new_gap > 0.0f)
            {
                g_gap_value = new_gap;
                fds_gapvalue_save(g_gap_value);
                NRF_LOG_INFO("GapValue updated: " NRF_LOG_FLOAT_MARKER, NRF_LOG_FLOAT(g_gap_value));
            }
        }
        else if (strncmp((char *)buf, "TARE", 4) == 0)
        {
            g_tare_requested = true;
            NRF_LOG_INFO("Tare requested via BLE");
        }
        else if (strncmp((char *)buf, "TEMP:", 5) == 0)
        {
            float new_temp = (float)atof((char *)buf + 5);
            g_temperature = new_temp;
            NRF_LOG_INFO("Temperature set: " NRF_LOG_FLOAT_MARKER, NRF_LOG_FLOAT(g_temperature));
        }
        else
        {
...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            ...
            nrf_gpio_pin_set(LED_B);          // 先熄灭
            app_timer_stop(m_led_timer_id);          // 先停止
...
int main(void)
{
    ...
    // 从Flash读取上次的GapValue
    float saved_gap = 0.0f;
    if (fds_gapvalue_load(&saved_gap))
    {
        g_gap_value = saved_gap;
        NRF_LOG_INFO("Restored GapValue from flash: " NRF_LOG_FLOAT_MARKER, NRF_LOG_FLOAT(g_gap_value));
    }
